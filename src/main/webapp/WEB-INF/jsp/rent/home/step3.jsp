		...
		<div class="text_box">
			...
			<ul class="answer_list answer_list_q3_mo">
				<c:forEach items="${rslt.answerList}" var="list" varStatus="i">
					<li><a href="javascript:void(0);" onclick="javascript:fn_selectMainStep('4', '<c:out value="${list.qtnId }"/>', '<c:out value="${list.asrId }"/>');">✓</a> <c:out value="${list.asrContents }" /></li>
...
if (ClickEventType == 'touchstart') {
	$('li a').on('touchstart', function(e) { // clear added stlye="" elements..
		$(this).css({
		 ...
		 fn_selectMainStep('4', '<c:out value="${list.qtnId }"/>', '<c:out value="${list.asrId }"/>');
	});
}
</script>